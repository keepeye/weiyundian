 ...
 * @param errorfun 失败回调
 */
function ajax(url,jsobject,successfun,errorfun,pobj){
	if(!pobj){
		pobj = window;
	}
	var async = true;
 ...
 * 列表对象的异步删除操作
 */
function datadel(url,o,met){	
	ajax(url,null,function(){var jobj = $(o);eval(met);});
}
/**
 * post方式提交分页带条件的分页
 * as:A链接标签
 * form:提交form
 */
function postpage(as,form){
	as.each(function(){
		$(this).data('form',form);
		$(this).attr('ohref',$(this).attr('href'));
		$(this).attr('href','javascript:;')
		$(this).click(function(){
			form.attr('action',$(this).attr('ohref'));
...
function YYUC_getnextsel(o,tn,aw){
	...
	if($.trim(o.value)!=''){
		ajax('/@system/ajax-getselvt.html',{tn:tn,pid:$(o).val(),aw:aw},function(m){
			for(var i=0;i<m.length;i++){
				sel.append('<option value="'+m[i].id+'">'+m[i].name+'</option>');
			}
		});
	}
}
 ...
 * @returns
 */
function pophtml(html,w,h,comefrom){
