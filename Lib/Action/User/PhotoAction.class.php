...
class PhotoAction extends UserAction{
	...
	public function list_add(){
		$photo=M('Photo')->where(array('token'=>$_SESSION['token'],'pid'=>$this->_get('pid')))->find();
		if($photo==false){$this->error('相册不存在');}
		if(IS_POST){			
			M('Photo')->where(array('token'=>session('token'),'id'=>$this->_post('pid')))->setInc('num');
			$this->all_insert('Photo_list','/list_add?id='.$this->_post('pid'));			
		}else{
			$data=M('Photo_list');
