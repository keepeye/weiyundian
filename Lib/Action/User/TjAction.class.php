...
class TjAction extends UserAction{
	...
	//默认统计
	function index(){
		$list = $this->_model->field("day,SUM(shares) as shares,SUM(clicks) as clicks")->where(array("token"=>$this->_token))->group("year,month,day")->limit(0,10)->order("id ASC")->select();//10天内的数据
