	...
	public function index(){
		...
		if(IS_POST){
			...
			if($cardinfo){ //如果Member_card_create 不为空，说明领过卡，但是可以修改会员信息
                ...
                $back = M('wxuser')->where(array('token'=>$this->_get('token')))->setInc('yetcardnum');
                $now_number=M('wxuser')->field('yetcardnum')->where(array('token'=>$this->_get('token')))->find();
                $data['number']="No.".$now_number['yetcardnum'];
                $data['wecha_id']=$this->_get('wecha_id');
                $data['token']=$this->_get('token');
